...
class WindidUserValidator
{
    ...
    public static function checkName($username, $uid = 0, $checkUsername = '')
    {
        ...
        if (self::isNameLenValid($username, Wekit::app('windid')->charset)) {
            return new WindidError(WindidError::NAME_LEN);
        }

        if (self::hasIllegalChar($username)) {
            return new WindidError(WindidError::NAME_ILLEGAL_CHAR);
        }
        ...
        if ($user = self::_getUserService()->getUserByName($username, WindidUser::FETCH_MAIN)) {
            ...
            if ($checkUsername && $user['username'] == $checkUsername) {
                return true;
            }
    ...
    {
        $len = strlen($password);
        if (self::getConfig('security.password.min') && $len < self::getConfig('security.password.min')) {
            return new WindidError(WindidError::PASSWORD_LEN);
        }
        if (self::getConfig('security.password.max') && $len > self::getConfig('security.password.max')) {
            return new WindidError(WindidError::PASSWORD_LEN);
        }
