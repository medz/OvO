...
//		$activeCodeDs->activeCode($this->info['uid'], Pw::getTime());
    ...
    {
        if (false === $this->isBindMail()) {
            return new PwError('USER:findpwd.notbind.email');
        }
    ...
    {
        if (false === $this->isBindMobile()) {
            return new PwError('USER:findpwd.notbind.mobile');
        }
