...
class PwInstall implements iPwInstall
{
    ...
    public function unInstall($uninstall)
    {
        ...
        if ($inject = $uninstall->getInstallLog('inject')) {
            $this->_loadPwHookInject()->batchDel(
            $inject);
        }
        if ($hooks = $uninstall->getInstallLog('hook')) {
            $this->_loadPwHooks()->batchDelByName(
            $hooks);
        }
        if ($appId = $uninstall->getInstallLog('appId')) {
