...
class PwMedalLog {
	...
	*/
	...
	public function updateExpiredByUidMedalId($uid, $medalId, $time) {
		...
		$medalId = (int)$medalId;
		$time = (int)$time;
		if ($uid < 1 || $medalId < 1) return false;
	...
	public function deleteInfosByUidMedalIds($uid, $medalIds = array()) {
		if ($uid < 1 || count($medalIds) < 1 ) return false;
