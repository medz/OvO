...
class PwUserValidator
{
    ...
    {
        if (0 >= preg_match('/^1\d{10}$/', $mobile)) {
            return new PwError('USER:mobile.error.formate');
        }
    ...
    public static function checkPwdComplex($password, $username)
    {
        ...
        if (in_array(9, $pwdConfig)) {
            ...
            if ($username == $password) {
                return new PwError('USER:pwd.error.equalUsername');
            }
        }
        if ($config == 0) {
            return true;
