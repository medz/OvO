...
class PwLikeTagDm extends PwBaseDm {
	...
	protected function _beforeAdd() {
		if (empty($this->_data['tagname'])) return new PwError('BBS:like.tagname.empty');
		return true;
	}
	
	protected function _beforeUpdate() {
		if ($this->tagid < 1) return new PwError('BBS:like.tagid.empty');
		if (empty($this->_data['tagname'])) return new PwError('BBS:like.tagname.not.empty');
		return true;
	}
}
?>