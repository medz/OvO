...
class PwTaskMemberFansDm extends PwTaskDm {
	...
	protected function filterConditionData() {
		...
		if (!$condition['num']) return new PwError('TASK:condition.fans.num.require');
		if (!WindValidator::isNonNegative($condition['num'])) return new PwError('TASK:condition.fans.num.isNonNegative');
