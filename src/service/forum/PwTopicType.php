    ...
    public function getTopicTypesByFid($fid, $filterAdmin = false)
    {
        ...
        if ($filterAdmin) {
            foreach ($topicTypes['all_types'] as $k => $v) {
                if ($v['issys']) {
                    if ($v['parentid'] > 0) {
                        if (isset($topicTypes['sub_topic_types'][$v['parentid']])) {
                            unset($topicTypes['sub_topic_types'][$v['parentid']][$k]);
                        }
                    } else {
                        unset($topicTypes['topic_types'][$k]);
                        if (isset($topicTypes['sub_topic_types'][$k])) {
                            unset($topicTypes['sub_topic_types'][$k]);
                        }
                    }
