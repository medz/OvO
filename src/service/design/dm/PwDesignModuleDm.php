...
class PwDesignModuleDm extends PwBaseDm {
	...
	public function __construct($moduleid = null) {
		if (isset($moduleid))$this->moduleid = (int)$moduleid;
	}
	...
	public function setStyle($font,$link,$border,$margin,$padding,$background,$styleclass) {
		...
		foreach ($array AS $v) {
			if (isset($border[$v]['linewidth']) && $border[$v]['linewidth'] !=='') $border[$v]['linewidth']= (int)$border[$v]['linewidth'];
		}
		...
		foreach ($array AS $v) {
			if (isset($margin[$v]) && $margin[$v] !=='') $margin[$v]= (int)$margin[$v];
		}
		...
		foreach ($array AS $v) {
			if (isset($padding[$v]) && $padding[$v] !=='') $padding[$v]= (int)$padding[$v];
		}
	...
	public function setCache($array) {
		...
		if ($array['start_hour'] > 23 || $array['start_hour'] < 0)  $array['start_hour'] = 0;
		if ($array['end_hour'] > 23 || $array['end_hour'] < 0)  $array['end_hour'] = 23;
		if ($array['start_minute'] > 59 || $array['start_minute'] < 0)  $array['start_minute'] = 0;
		if ($array['end_minute'] > 59 || $array['end_minute'] < 0)  $array['end_minute'] = 59;
	...
	protected function _beforeUpdate() {
		if ($this->moduleid < 1) return new PwError('operate.fail');
		return true;
	}
