...
class PwDesignData {
	...
	public function fetchData($dataids) {
		if (!is_array($dataids)) return array();
	...
	public function fetchDataByFrom($fromids, $fromtype = self::FROM_AUTO, $datatype = self::AUTO) {
		if (!is_array($fromids)) $fromids =  (int)$fromids > 0 ? array($fromids) : array();
		if (count($fromids) < 1) return array();
	...
	public function fetchDataByModuleid($moduleids) {
		if (!is_array($moduleids) || !$moduleids) return array();
	...
	public function getMinDataIdByOrder($moduleid, $orderid) {
		$moduleid = (int)$moduleid;
		$orderid = (int)$orderid;
		if ($moduleid < 1 || $orderid < 1) return 0;
	...
	public function updateOrder($dataid, $orderid) {
		$dataid = (int)$dataid;
		$orderid = (int)$orderid;
		if ($dataid < 1 || $orderid < 0) return false;
	...
	public function batchDelete($dataids) {
		if (!is_array($dataids)) $dataids =  (int)$dataids > 0 ? array($dataids) : array();
		if ( count($dataids) < 1 ) return false;
