...
```shell
curl -v -H "Accept: application/json" https://fans.io/api/v1/login
```
...
```shell
curl -i "https://fans.io/api/v1/users?limit=10"
```

对于 POST 、 PATCH 、 PUT 和 DELETE 请求， URL 中没有包含的参数应该为请求体的 JSON，其中，请求头应该设置 `Content-Type` 为 `application/json` ：

```shell
curl -i -d '{"login": ":username", "password": ":password"}' https://fans.io/api/v1/login
```

<a name="http-redirects"></a>
## HTTP 重定向
...
```shell
curl -i https://fans.io/api/v1/users/1
HTTP/1.1 200 OK
...
```
https://fans.io/api/v1/user?token={TOKEN}
```
