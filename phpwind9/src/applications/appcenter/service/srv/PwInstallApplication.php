...
class PwInstallApplication
{
    ...
    public function onlineInstall($id)
    {
        ...
        if ($manifest instanceof PwError) {
            return $manifest;
        }
    ...
    {
        if (true !== ($_r = $this->extractPackage($installPack))) {
            return $_r;
        }
        if (true !== $_r = $this->initInstall()) {
            return $_r;
        }
        if (true !== $_r = $this->doInstall('all', $this->_hash)) {
            $this->rollback();
    ...
    public function doInstall($step, $hash)
    {
        try {
            ...
            if (empty($service)) {
                return new PwError('APPCENTER:install.service.fail');
            }
